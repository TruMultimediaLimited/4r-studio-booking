...
function HeaderCard({ href, onClick, icon: Icon, label }) {
  ...
  return (
    <Tag
      {...linkProps}
      className="flex items-center justify-center gap-1 w-20 bg-white border border-[#E0E0E0] rounded-lg px-1.5 py-1 shadow-sm hover:border-pine/40 hover:bg-pine/5 hover:shadow-md transition-all"
    >
      <Icon className="h-2.5 w-2.5 text-pine shrink-0" />
      <span className="text-[9px] font-medium text-[#333333]/75 whitespace-nowrap">{label}</span>
    </Tag>
  )
}
    ...
    <div className={`min-h-screen font-body ${isAdmin ? 'bg-paper' : 'bg-[#F9F7F2]'}`}>
      ...
      >
        {isAdmin ? (
          ...
          <div className="font-sans max-w-5xl mx-auto px-4 py-1.5 grid grid-cols-3 items-center gap-1.5">
            ...
            <div className="flex items-center justify-center min-w-0">
              <div className="bg-white border border-[#E0E0E0] rounded-lg shadow-sm px-2.5 py-1">
