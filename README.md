...
| `WEBHOOK_SECRET` | নিজে একটা লম্বা র‍্যান্ডম ভ্যালু বানান — টার্মিনালে `openssl rand -hex 24` চালালেই পাবেন |

(`WEBHOOK_SECRET`-টা একটা পাসওয়ার্ডের মতো — **কখনোই এই README বা কোডে লিখে রাখবেন না**, শুধু Vercel আর Supabase-এর সেটিংসে বসাবেন। আগে এই ফাইলে একটা আসল ভ্যালু লেখা ছিল এবং সেটা git হিস্টরিতে রয়ে গেছে — তাই এখনই একটা **নতুন** ভ্যালু বানিয়ে দুই জায়গাতেই (Vercel + Supabase, নিচে) বদলে নিন।)
...
7. **HTTP Headers**-এ একটা নতুন হেডার যোগ করুন: নাম `x-webhook-secret`, ভ্যালু Vercel-এ যেই `WEBHOOK_SECRET` দিয়েছেন হুবহু সেটাই।
8. Save করুন।
